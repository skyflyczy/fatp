	...
	public void genPayinvestListExcel(List<BizplanPayinvest> list, OutputStream os,ListingInfo listingInfo) throws IOException{
		...
		if(CollectionUtils.isNotEmpty(list)) {
			for(int i = 0 ; i < list.size() ; i ++) {
				...
				//收款人名称栏：必填，最大90个汉字
				currentRow.createCell(3).setCellValue(payinvest.getInvestUserRealName());
				...
				currentRow.createCell(6).setCellValue(payinvest.getSubBankName());
				//金额栏：必填，最大百亿位，可以有2位小数位，也可没有小数位
				currentRow.createCell(7).setCellValue(payinvest.getPrincipal().add(payinvest.getInterest()).doubleValue());
				//预约转账标志，非必填，不填留空，要预约转账时填Y
				currentRow.createCell(8).setCellValue("");
				//TODO 用途，必输，请输入款项真实用途，企业对用途的真实性负责，最大50位
				currentRow.createCell(9).setCellValue("兑付");
