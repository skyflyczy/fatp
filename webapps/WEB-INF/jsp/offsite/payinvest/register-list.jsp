...
<div class="bjui-pageHeader" id="fasset-prolist-search-search">
    <form id="pagerForm" data-toggle="ajaxsearch" action="<%=request.getContextPath()%>/offsite/payinvest/register-list.do" method="post">
        ...
        <div class="bjui-moreSearch">
			...
			<input type="text" name="auditTimeBegin" value="${search.auditTimeBegin }" class="form-control" size="10" data-toggle="datepicker"
				   data-pattern="yyyy-MM-dd" data-nobtn="true" data-rule="date"/>
			<label>-</label>
			<input type="text" name="auditTimeEndText" value="${search.auditTimeEndText }" class="form-control" size="10" data-toggle="datepicker"
				   data-pattern="yyyy-MM-dd" data-nobtn="true" data-rule="date"/>
        </div>
    </form>
</div>
    ...
    <table data-toggle="tablefixed">
    	<thead>
	    	<tr>
				...
				<th width="22%">挂牌名称</th>
				<th width="18%">投顾简称</th>
				<th align="right" width="12%">兑付总额(元)</th>
				<th align="center" width="6%">兑付期数</th>
				<th align="center" width="8%">兑付日期</th>
				<!--
				<th align="center" width="12%">提交时间</th>
				-->
				<th align="center" width="12%">审核时间</th>
    	...
    	<tbody>
			...
			<tr align="center">
	    		...
	    		<td align="right"><fmt:formatNumber value="${list.payMoney}" pattern="#,##0.00"/></td>
	    		<td>${list.periodNumber}</td>
	    		<td><fmt:formatDate value="${list.payinvestTime}" pattern="yyyy-MM-dd"/></td>
	    		<!--
	    		...
	    		<td>	    		
    			<c:choose>
	    			<c:when test="${list.applyStatus == 8 ||list.applyStatus == 9}"><a href="javascript:;" data-toggle="alertmsg"  data-title="审核意见" data-type="info" data-msg="${list.auditRemark eq null?' ':list.auditRemark}" data-ok-name="关闭" data-auto-close="false" >${list.applyStatusDesc }</a></c:when>
	    			<c:otherwise>${list.applyStatusDesc }</c:otherwise>
	    		</c:choose>	    		
	    		</td>
	    		<td><a href="<%=request.getContextPath()%>/offsite/payinvest/view_apply.do?applyId=${list.applyId}"
	    				data-toggle="dialog" data-width="1000"  data-height="600" data-id="dialog-mask"
