...
<div class="bjui-pageHeader" id="fasset-prolist-search-search">
    <form id="pagerForm" data-toggle="ajaxsearch" action="<%=request.getContextPath()%>/offsite/transfer/firstaudit-list.do" method="post">
        ...
        <div class="bjui-searchBar">
			...
			<select name="productStatus" id="productStatus" data-toggle="selectpicker" class="show-tick">
				<option value="">-产品到期-</option>
				<option value="1" <c:if test="${search.productStatus == 1 }">selected</c:if>>存续期内</option>
				<option value="2" <c:if test="${search.productStatus == 2 }">selected</c:if>>已到期</option>
			</select>
			 --%>
