...
<div class="bjui-pageHeader" id="fasset-prolist-search-search">
    <form id="pagerForm" data-toggle="ajaxsearch" action="<%=request.getContextPath()%>/offsite/transfer/register-list.do" method="post">
        ...
        <div class="bjui-searchBar">
			...
			<%--
			<select name="productStatus" id="productStatus" data-toggle="selectpicker" class="show-tick">
				<option value="0" >-产品到期-</option>
				<option value="1"  <c:if test="${search.searchProjectMode == 1}">selected</c:if>>存续期内</option>
				<option value="2"  <c:if test="${search.searchProjectMode == 2}">selected</c:if>>已到期</option>
			</select>
			 --%>
			...
			<input type="text" name="searchProjectKeyword" value="<c:out value='${search.searchProjectKeyword}'/>" class="form-control" size="20" />
			&nbsp;&nbsp;&nbsp;
			...
			<input type="text" name="timeBegin" value="${search.timeBegin }" class="form-control" size="10" data-toggle="datepicker"
				   data-pattern="yyyy-MM-dd HH:mm:ss" data-nobtn="true" data-rule="datetime"/>
			<label>-</label>
			<input type="text" name="timeEnd" value="${search.timeEnd }" class="form-control" size="10" data-toggle="datepicker"
				   data-pattern="yyyy-MM-dd HH:mm:ss" data-nobtn="true" data-rule="datetime"/>
    ...
    <table data-toggle="tablefixed">
    	<thead>
	    	<tr>
				...
				<!--
				<th align="center" width="18%">转让时间段</th>
				<th align="center" width="10%">申请创建时间</th>
				-->
				<th align="center" width="12%">登记更新时间</th>
    	...
    	<tbody>
	    	...
	    	<tr align="center">
	    		...
	    		<td align="left"><span class="text-omit pull-left">${list.investMemberName}</span></td>
	    		...
	    		<td>
	    			<c:choose>
		    			<c:when test="${list.applyStatus == 9 }"><a href="javascript:;" data-toggle="alertmsg"  data-title="登记失败原因" data-type="info" data-msg="${list.auditRemark eq null?' ':list.auditRemark}" data-ok-name="关闭" data-auto-close="false" >${list.applyStatusDesc }</a></c:when>
		    			<c:otherwise>${list.applyStatusDesc }</c:otherwise>
		    		</c:choose>
	    		</td>
	    		<td>
	    			<a href="<%=request.getContextPath()%>/offsite/transfer/view_apply.do?applyId=${list.applyId}"
	    				data-toggle="dialog" data-width="1000"  data-height="600" data-id="dialog-mask"
...
function showReason(reason) {
	$(this).alertmsg('info', reason, {title:'登记失败原因',autoClose:'false',okName:'关闭'});
}
</script>