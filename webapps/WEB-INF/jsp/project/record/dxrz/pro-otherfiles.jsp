...
<div class="bjui-pageContent tableContent">
    <form  action="" id="j_custom_form" class="pageForm" data-toggle="validate" method="post">
    	<table id="tabledit2" data-reload-navtab="true" class="table table-hover" data-toggle="tabledit" data-initnum="0" data-action="<%=request.getContextPath()%>/debt/proissue/profiles/edit.do?projectId=${projectId}&belongType=1" data-single-noindex="true" data-callback="afterEdit3">
           	<col style="width:8%" />
			<col style="width:12%" />
			<col style="width:20%" />
			<col style="width:15%" />
        		...
        		<tr align="center">
                    ...
                    <td data-noedit="true">
                    	<div class="inline-upload"
                        	data-toggle="upload"  
                            data-uploader="//<%=request.getServerName() %>/debt/proissue/profiles/upload.do" 
        ...
        <table id="tabledit3" data-reload-navtab="true" class="table table-hover" data-toggle="tabledit" data-initnum="0" data-action="<%=request.getContextPath()%>/debt/proissue/profiles/edit.do?projectId=${projectId}&belongType=1" data-single-noindex="true" data-callback="afterEdit3">
           	<col style="width:8%" />
			<col style="width:12%" />
			<col style="width:20%" />
			<col style="width:15%" />
			...
			<col style="width:18%" />
           	<thead style="display:none">
                <tr>
                    <th data-noedit="true" data-align="center"><input type="text" name="id"  size="2" readonly><input type="hidden" name="index" value="#index#"></th>
            ...
            <c:forEach var="obj" items="${list}" varStatus="status"> 
                <tr align="center">
                    ...
                    <td data-noedit="true">
                        ...
                        <button type="button" class="btn-blue" data-toggle="doedit" onclick="showUpload3(${ status.index + 1})">编辑</button>
                        <a href="<%=request.getContextPath()%>/debt/proissue/profiles/delete.do?id=${obj.id }&belongType=1" class="btn btn-red row-del" data-confirm-msg="确定要删除该行信息吗？" data-callback="afterDel3">删除</a>
                    </td>
                </tr>
			</c:forEach>
            </tbody>
	   	...
	   	if(!json.hasSeal  || !json.sealSessionId) {
	    	...
	    	obj.val(json.fileName);
	   		...
	   		if(($.getExplorerInfo.type=='Chrome' && $.getExplorerInfo.platform=='32' && (ver>=28 && ver<=41)) || ($.getExplorerInfo.type=='IE' && (ver>=8 && ver<=11))){
	 				...
	 				//进行签名
	 				 $.post(
			    		"<%=request.getContextPath()%>/debt/proissue/profiles/dofilesign.do",
...
function delFile3($element){
	...
	if(picNames !='undefiend' && picNames !=""){
		...
		$.post("<%=request.getContextPath()%>/debt/proissue/profiles/delfile.do",options,function(data){
		});
	}
}
</script>
